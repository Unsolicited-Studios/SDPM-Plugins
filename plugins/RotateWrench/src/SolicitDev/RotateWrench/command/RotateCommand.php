...
class RotateCommand extends BaseCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        if (RotateWrench::rotateBlock($block, Facing::opposite($sender->getHorizontalFacing()))) {
            $sender->sendMessage('Block rotated! Block: ' . $block->getName() . ' (' . $block->getId() . ':' . $block->getMeta() . ')');
            return;
        }
        $sender->sendMessage('Failed to rotate block! May be possible that this block has no facing trait.');
    }
}
